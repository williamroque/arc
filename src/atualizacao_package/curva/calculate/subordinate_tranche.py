...
def carencia_phase(self, F_i, tranche_list, tranche_i):
    if self.i - self.phase_first_index < self.inputs.get('curve')['c-period']:
        ...
        pmt = 0
        amort = 0
        saldo = self.saldo + self.inputs.get('curve')['despesas'] + juros - pmt
...
def dependent_phase(self, F_i, tranche_list, tranche_i):
    juros = self.saldo * self.taxa_juros
    pmt = juros + self.inputs.get('curve')['despesas']
    amort = pmt - juros - self.inputs.get('curve')['despesas']
    saldo = self.saldo + self.inputs.get('curve')['despesas'] + juros - pmt
...
def final_phase(self, F_i, tranche_list, tranche_i):
    juros = self.saldo * self.taxa_juros
    amort = self.saldo
    pmt = self.inputs.get('curve')['despesas'] + juros + amort
    saldo = self.saldo + self.inputs.get('curve')['despesas'] + juros - pmt
